...
// Force Node.js to accept Supabase's certificate chain
// Required since Supabase uses pooler certs that Node 18+ rejects
...
export async function extendReservation(reservationId, minutes = 5) {
  const result = await query(
    `UPDATE reservations
     SET expires_at = NOW() + INTERVAL '1 minute' * $2
...
/**
 * Update order status with timestamp tracking and optional extras
 */
export async function updateOrderStatus(orderId, status, extras = {}) {
  const { carrier, trackingNumber, estimatedDelivery, labelUrl, notes } = extras;
  ...
  if (status === 'processing') updates.push('processing_at = NOW()');
...
/**
 * Create an order event (timeline entry)
 */
export async function createOrderEvent(orderId, { status, message, carrier, trackingNumber, location, source = 'admin' }) {
  const result = await query(
    `INSERT INTO order_events (order_id, status, message, carrier, tracking_number, location, source)
     VALUES ($1, $2, $3, $4, $5, $6, $7)
     RETURNING *`,
    [orderId, status, message || null, carrier || null, trackingNumber || null, location || null, source]
  );
  return result.rows[0];
}

/**
 * Get order tracking info with timeline events
 */
export async function getOrderTracking(orderNumber) {
  const orderResult = await query(
    `SELECT o.*, c.name as clawd_name
     FROM orders o
  ...
  return { order, events: eventsResult.rows };
}
...
// ============================================================================
// DESIGN CREDITS (Share-to-Earn)
// ============================================================================

/**
 * Get design credits for a wallet (creates with 1 free credit if new)
 */
export async function getDesignCredits(walletAddress) {
  const result = await query(
    `INSERT INTO design_credits (wallet_address, credits, total_earned)
     VALUES ($1, 1, 1)
  ...
  const existing = await query(
    'SELECT * FROM design_credits WHERE wallet_address = $1',
    [walletAddress]
  );
  return existing.rows[0] || { credits: 1, total_earned: 1, total_spent: 0 };
}

/**
 * Spend one design credit
 */
export async function spendDesignCredit(walletAddress) {
  const result = await query(
    `UPDATE design_credits
     SET credits = credits - 1, total_spent = total_spent + 1, updated_at = NOW()
     WHERE wallet_address = $1 AND credits > 0
     RETURNING *`,
    [walletAddress]
  );
  return result.rows[0] || null;
...
/**
 * Add design credits (earned via social sharing)
 */
export async function addDesignCredits(walletAddress, amount = 1) {
  const result = await query(
    `INSERT INTO design_credits (wallet_address, credits, total_earned)
     VALUES ($1, 1 + $2, 1 + $2)
     ON CONFLICT (wallet_address) DO UPDATE
     SET credits = design_credits.credits + $2,
         total_earned = design_credits.total_earned + $2,
         updated_at = NOW()
     RETURNING *`,
